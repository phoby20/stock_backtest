...
const CANDLE_META: Record<
  string,
  { label: string; maxPeriod: string; autoLookback: number }
> = {
...
function calcMACD(closes: number[], fast: number, slow: number, sig: number) {
  const macd = emaSpan(closes, fast).map(
    (f, i) => f - emaSpan(closes, slow)[i],
  );
...
function generateSignalsRsiMacd(
  ...
  for (let i = 1; i < n; i++) {
    ...
    const falling =
      rsi[i] !== null &&
      rsi[i - 1] !== null &&
      (rsi[i] as number) < (rsi[i - 1] as number);
...
function runBacktest(
  ...
  if (position && shares > 0) {
    ...
    trades.push({
      ...
      amount: round(capital, 2),
      commission: round(commission, 2),
      rsi: round(rsi[data.length - 1] ?? 0, 2),
    });
...
export async function POST(req: NextRequest) {
  try {
    ...
    const data: OHLCVRow[] = rawData.map((r) => ({
    ...
    const {
      ...
      signal: macdSig,
      hist,
    } = calcMACD(closes, macd_fast, macd_slow, macd_signal);

    const signals =
      strategy === "rsi-macd"
        ? generateSignalsRsiMacd(
            rsi,
            macd,
            macdSig,
            oversold,
    ...
    console.error(e);
    return NextResponse.json(
      { detail: e instanceof Error ? e.message : "서버 오류" },
      { status: 400 },
    );
  }
}
