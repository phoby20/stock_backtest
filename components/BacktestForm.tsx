...
const LABEL_CLS =
  "block text-xs font-semibold text-gh-muted uppercase tracking-wide mb-1";
...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-2 pt-1">
      <span className="text-xs font-semibold text-gh-muted uppercase tracking-wide">
        {children}
      </span>
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <Field label="종목코드">
        <input
          ...
          className={INPUT_CLS}
          placeholder="예: AAPL, SOXL, 005930.KS"
          required
        />
      </Field>
        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              <div key={label}>
                <label className="block text-xs text-gh-muted mb-1">
                  {label}
                </label>
                <input
                  type="number"
